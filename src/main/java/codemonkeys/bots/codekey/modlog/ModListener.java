...
public class ModListener extends EventListenerAdapter {
	...
	public void Ready(ReadyEvent e) {
		System.out.println("Does this thing even run at all?");
		System.out.println(Main.config.modLogs.guildID);
		DataBase.connect();
	}
	...
	public void GuildBanRemove(GuildBanRemoveEvent e) {
		...
		e.getGuild().getAuditLog(ActionTypes.MEMBER_BAN_REMOVE, 1).thenAcceptAsync(aLogs -> {
			if (aLogs.getEntries().size() > 1) {
				...
				long caseNumber = getNextCaseNumber();
				final String reason = entry.getReason() == null ? String.format("No reason provided. Use `!reason <%d> <reason>` to change the reason.", caseNumber) : entry.getReason();
				RichEmbed embed = new RichEmbed("Member Unbanned").setAuthor(entry.getAuthor().toString(), "", entry.getAuthor().getAvatar().toString());
		...
		cf.thenAcceptAsync(aLogs -> {
			...
			if (aLogs.getEntries().size() > 1) {
				...
				if (entry.getTargetID() != e.getMember().getID()) { // make sure that the member was actually kicked
					System.out.println("not same. " + entry.getTargetID() + ": " + e.getMember().getID());
					return; // and return early if they weren't
				}
				long caseNumber = getNextCaseNumber();
				final String reason = entry.getReason() == null ? String.format("No reason provided. Use `!reason <%d> <reason>` to change the reason.", caseNumber) : entry.getReason();
				RichEmbed embed = new RichEmbed("Member Kicked").setAuthor(entry.getAuthor().toString(), "", entry.getAuthor().getAvatar().toString());
